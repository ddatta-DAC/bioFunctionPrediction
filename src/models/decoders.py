...
def calc_performance_metrics(labels, predictions, threshold=0.35):
    ...
    tp = tf.reduce_sum(
        tf.cast(
            tf.logical_and(labels, predictions),
            tf.float32
        ),
        axis=1
    )
    fp = tf.reduce_sum(
        tf.cast(
            tf.logical_and(tf.logical_not(labels), predictions), tf.float32
        ),
        axis=1
    )
    fn = tf.reduce_sum(
        tf.cast(
            tf.logical_and(labels, tf.logical_not(predictions)), tf.float32
        ),
        axis=1)
...
class HierarchicalGODecoder(object):
    ...
    def build(self, godag):
        ...
        self.loss = tf.reduce_mean(
            self.lossfunc(
                labels=self.ys_[:, :len(self.funcs)],
                logits=logits
            )
        )
