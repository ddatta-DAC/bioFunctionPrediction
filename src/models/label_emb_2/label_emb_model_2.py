...
__version__ = "0.0.1"
__processor__ = 'label_emb_model_2'
...
try:
    ...
    from models.conv_autoencoder_v1_0 import ConvAutoEncoder
    from models.label_emb_2.joint_inf_decoder import joint_inf_decoder
    ...
    from bioFunctionPrediction.src.models.label_emb_2.joint_inf_decoder import joint_inf_decoder
    import bioFunctionPrediction.src.utils.dataloader as dataloader
# ---------------- #
...
node2vec_emb_file = 'goEmbeddings.txt'
...
def get_pretrained_cae():
    ...
    model_name_prefix = 'label_emb_2_cae'
...
def get_node2vec_emb():
    global n2v_emb_dim
    global node2vec_emb_file
    n2v_path = os.path.join(FLAGS.resources, node2vec_emb_file)
    emb_dict = dataloader.load_labelembedding_node2vec(n2v_path)
    dummy_key = list(emb_dict.keys())[0]
    n2v_emb_dim = emb_dict[dummy_key].shape[0]
    return emb_dict
...
def create_label_emb_lookup():
    ...
    n2v_emb_dict = get_node2vec_emb()
    # format the keys to add 'GO:'
    n2v_emb_dict = {
        'GO:' + k: v for k, v in n2v_emb_dict.items()
    }
    ...
    _idmap = GODAG.idmap
    yid_vec_lookup = np.zeros([label_count + 1, n2v_emb_dim])

    for i in range(label_count):
        idx = _idmap[target_funcs[i]]
        try:
            yid_vec_lookup[idx] = n2v_emb_dict[target_funcs[i]]
        except:
            pass
...
def build_model():
    ...
    model_savename = 'label_emb_2_{}_{}'.format(int(time.time()),FLAGS.function)
...
def predict_only():
    ...
    model_name_prefix = 'label_emb_2_{}'.format(FLAGS.function)
